...
def getRRCTopN(date):
	sqlStr = 'select * from tle_topn.rrc_conn_h where BEGINCOLLECTTIME >= \'%s 00:00\' and BEGINCOLLECTTIME <= \'%s 23:59\''%(date.strftime('%Y-%m-%d'), date.strftime('%Y-%m-%d'))
	cnx = connector.connect(host='132.110.60.94', user='root', password='Abcdef9*')
	cursor = cnx.cursor()
